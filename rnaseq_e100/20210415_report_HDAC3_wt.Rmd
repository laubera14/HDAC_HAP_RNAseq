---
title: 'HAP FFG RNA-seq: HDAC3 introduction (wt) comparison to inhibitor treatment and KO Top90 (updated to hg38_e100)'
...
# Background & Summary
Comparison of the difference between introduction of inactive HDAC3 Mutant into wt and inhibitors. And additionally comparison to KO generation?
...
create_heatmap <- function(inp_list, n_genes, title, out_file, sort_decr = TRUE, clustered = TRUE, numbers = FALSE) {
  ...
  betascols <- c("intercept", "HDAC3KO", "JQ12 1µM", "MS275 1µM", "MS275 3µM", "HDAC3wt HDAC3Mut")
  ...
  col.order <- c("HDAC3wt HDAC3Mut", "MS275 1µM", "MS275 3µM", "JQ12 1µM", "HDAC3KO")
  mat <- mat[,col.order]
...
create_heatmap_inh <- function(inp_list, n_genes, title, out_file, sort_decr = TRUE, clustered = TRUE, numbers = FALSE) {
  ...
  col.order <- c("MS275 1µM", "HDAC3wt HDAC3Mut", "HDAC3KO") #"MS275 3µM", "JQ12 1µM")
  mat <- mat[,col.order]
...
cts <- cts_full %>% select(gene_id, contains(c("HAP_135_", "HAP_1_", "HAP_2_", "HAP_3_", "HAP_JQ", "HAP_MS1_", "HAP_MS3", "HD3_KO"))) %>% column_to_rownames("gene_id")
...
coldata$expression <- factor(coldata$expression, levels = c("native", "iHDAC3"))
coldata$group <- factor(coldata$group, levels = c("wt_native", "HDAC3ko_native", "wt_JQ12_1uM_24h", "wt_MS275_1uM_24h",
                                                  "wt_MS275_3uM_24h", "wt_inactive_HDAC3"))
```
...
dds$group <- factor(dds$group, levels = c("wt_native", "HDAC3ko_native", "wt_JQ12_1uM_24h", "wt_MS275_1uM_24h",
                                          "wt_MS275_3uM_24h", "wt_inactive_HDAC3"))
```
...
plotMA(ci_vs_ctrl_all, ylim = c(-10,10), main = "group HDAC3wt HDAC3Mut vs wt native \n rowsum > 5")
...
```{r KO_significants}
KO_vs_ctrl_all <- results(dds, contrast = c("group", "HDAC3ko_native", "wt_native"))
...
for (listname in out_lists) {
  write_xlsx(rownames_to_column(as.data.frame(get(listname)), "gene_id"),
           path = paste0("20210415_HDACwt_ci_KO_inh_report/HDAC3_", listname, ".xlsx"))
}
...
write_xlsx(usefull_ratios, path = paste0("20210415_HDACwt_ci_KO_inh_report/HDAC3_ratios.xlsx"))
all_ratios = column_to_rownames(all_ratios, "comparison")

gene_numbers = data.frame(out_lists, out_lists_counts)
write_xlsx(gene_numbers, path = paste0("20210415_HDACwt_ci_KO_inh_report/HDAC3_GeneCounts.xlsx"))
...
# Heatmaps of HDAC3Mut introduction
...
This part contains the heatmaps which rank the genes according to regulation in the HDAC3Mut introduction.

## i) Top genes in HDAC3wt HDAC3Mut (general up&down)
### complete Heatmap
```{r gene_heatmap_up, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap(ci_vs_ctrl_up, 10000, "upregulated upon ci intro", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up.pdf")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up.pdf")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_up_from_ci_vs_ctrl_all",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up.xlsx)
...
res_table <- create_heatmap(ci_vs_ctrl_down, 10000, "downregulated upon ci intro",
                            "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down.pdf", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down.pdf")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_down_from_ci_vs_ctrl_all",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down.xlsx)

### Top90 with numbers
```{r gene_heatmap_up90n, out.width = "50%"}
res_table <- create_heatmap(ci_vs_ctrl_up, 90, "upregulated upon ci intro", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90n.png",
                            numbers = TRUE, clustered = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90n.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90n.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_up_from_ci_vs_ctrl_all",],"%")
```
...
res_table <- create_heatmap(ci_vs_ctrl_down, 90, "downregulated upon ci intro",
                            "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90n.png", sort_decr = FALSE, numbers = TRUE, clustered = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90n.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90n.png")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_down_from_ci_vs_ctrl_all",],"%")
```
...
res_table <- create_heatmap(ci_vs_ctrl_up, 90, "upregulated upon ci intro", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90.png")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_up_from_ci_vs_ctrl_all",],"%")
```
...
res_table <- create_heatmap(ci_vs_ctrl_down, 90, "downregulated upon ci intro",
                            "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90.png", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90.png")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_down_from_ci_vs_ctrl_all",],"%")
```
...
res_table <- create_heatmap(ci_vs_ctrl_up, 90, "upregulated upon ci intro", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90rank.png",
                            clustered = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90rank.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up90rank.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_up_from_ci_vs_ctrl_all",],"%")
```
...
res_table <- create_heatmap(ci_vs_ctrl_down, 90, "downregulated upon ci intro",
                            "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90rank.png", sort_decr = FALSE, clustered = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90rank.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down90rank.png")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_vs_ctrl_down_from_ci_vs_ctrl_all",],"%")
```
...
## ii) Top genes (up) in HDAC3wt HDAC3Mut when not upregulated in KO
### complete heatmap
```{r ci_up_KO_unchdown_vs_ctrl, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap(ci_up_KO_unchdown_vs_ctrl, 10000, "upregulated upon ci intro \n no upregulation in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown.pdf")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown.pdf")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_unchdown_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown.xlsx)

### Top90 clustered
```{r ci_up_KO_unchdown_vs_ctrl90, out.width = "50%"}
res_table <- create_heatmap(ci_up_KO_unchdown_vs_ctrl, 90, "upregulated upon ci intro \n no upregulation in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown90.png")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown90.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_unchdown_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_unchdown90.xlsx)

## ii) Top genes (down) in HDAC3wt HDAC3Mut when not downregulated in KO
### complete heatmap
```{r ci_down_KO_unchup_vs_ctrl, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap(ci_down_KO_unchup_vs_ctrl, 10000, "downregulated upon ci intro \n no upregulation in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup.pdf", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup.pdf")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_unchup_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup.xlsx)

### Top90 clustered
```{r ci_down_KO_unchup_vs_ctrl90, out.width = "50%"}
res_table <- create_heatmap(ci_down_KO_unchup_vs_ctrl, 90, "downregulated upon ci intro \n no upregulation in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup90.png", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup90.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_unchup_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_down_KO_unchup90.xlsx)

## iii) Top genes in HDAC3wt HDAC3Mut which are also upregulated in KO
### complete heatmap
```{r ci_up_KO_up_vs_ctrl, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap(ci_up_KO_up_vs_ctrl, 10000, "upregulated upon ci intro \n upregulated in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up.pdf")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up.pdf")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_up_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up.xlsx)

### Top90 clustered
```{r ci_up_KO_up_vs_ctrl90, out.width = "50%"}
res_table <- create_heatmap(ci_up_KO_up_vs_ctrl, 90, "upregulated upon ci intro \n upregulated in KO", "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up90.png")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up90.png")
message("genes: ", length(rownames(res_table)))
message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_up_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ci_up_KO_up90.xlsx)

## iii) Top genes in HDAC3wt HDAC3Mut which are also downregulated in KO
### complete heatmap
```{r ci_down_KO_down_vs_ctrl, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap(ci_down_KO_down_vs_ctrl, 10000, "downregulated downon ci intro \n downregulated in KO", "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down.pdf", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down.pdf")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_down_vs_ctrl_from_ci_vs_ctrl_down",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down.xlsx)

### Top90 clustered
```{r ci_down_KO_down_vs_ctrl90, out.width = "50%"}
res_table <- create_heatmap(ci_down_KO_down_vs_ctrl, 90, "downregulated downon ci intro \n downregulated in KO", "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down90.png", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down90.png")
message("genes: ", length(rownames(res_table)))
message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_down_vs_ctrl_from_ci_vs_ctrl_down",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ci_down_KO_down90.xlsx)
...
res_table <- create_heatmap_inh(ms275_vs_ctrl_up, 10000, "upregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up.pdf", clustered = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up.pdf")
message("genes: ", length(rownames(res_table)))
#message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
#message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_up_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ms275_vs_ctrl_up.xlsx)

### Top90 clustered
```{r ms275_up90, out.width = "50%"}
res_table <- create_heatmap_inh(ms275_vs_ctrl_up, 90, "upregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90.png")
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90.png")
message("genes: ", length(rownames(res_table)))
#message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
#message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_up_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ms275_vs_ctrl_up90.xlsx)

### Top90 with numbers
```{r ms275_up90n, out.width = "50%"}
res_table <- create_heatmap_inh(ms275_vs_ctrl_up, 90, "upregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90n.png", clustered = FALSE, numbers = TRUE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90n.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_up/HDAC3_ms275_vs_ctrl_up90n.png")
message("genes: ", length(rownames(res_table)))
#message("Total upregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_up",])
#message("percentage from all deregulated genes: ", all_ratios["ci_up_KO_up_vs_ctrl_from_ci_vs_ctrl_up",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_up/HDAC3_ms275_vs_ctrl_up90n.xlsx)


## i) general (down)
### complete heatmap
```{r ms275_down, out.height = "600px", out.width = "70%"}
res_table <- create_heatmap_inh(ms275_vs_ctrl_down, 10000, "downregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down.pdf", clustered = FALSE, sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down.pdf")
message("genes: ", length(rownames(res_table)))
#message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
#message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_down_vs_ctrl_from_ci_vs_ctrl_down",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ms275_vs_ctrl_down.xlsx)

### Top90 clustered
```{r ms275_down90, out.width = "50%"}
res_table <- create_heatmap_inh(ms275_vs_ctrl_down, 90, "downregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90.png", sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90.png")
message("genes: ", length(rownames(res_table)))
#message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
#message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_down_vs_ctrl_from_ci_vs_ctrl_down",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ms275_vs_ctrl_down90.xlsx)

### Top90 with numbers
```{r ms275_down90n, out.width = "50%"}
res_table <- create_heatmap_inh(ms275_vs_ctrl_down, 90, "downregulated by MS275 treatment", "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90n.png", clustered = FALSE, numbers = TRUE, sort_decr = FALSE)
write_xlsx(rownames_to_column(as.data.frame(res_table), "gene_name"),
           path = "20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90n.xlsx")
include_graphics("20210415_HDACwt_ci_KO_inh_report/ms275_down/HDAC3_ms275_vs_ctrl_down90n.png")
message("genes: ", length(rownames(res_table)))
#message("Total downregulated in HDAC3wt HDAC3Mut: ", gene_numbers["ci_vs_ctrl_down",])
#message("percentage from all deregulated genes: ", all_ratios["ci_down_KO_down_vs_ctrl_from_ci_vs_ctrl_down",],"%")
```
[download Excel Table](20210415_HDACwt_ci_KO_inh_report/ci_down/HDAC3_ms275_vs_ctrl_down90n.xlsx)
